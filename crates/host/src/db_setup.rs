...
async fn start_embedded(database_url: &str) -> Result<(PgEmbed, String), DbSetupError> {
    ...
    let user = {
        ...
        if u.is_empty() { "ecto_ledger" } else { u }.to_string()
    };
    let password = parsed.password().unwrap_or("ecto_ledger").to_string();
    let db_name = {
        let p = parsed.path().trim_start_matches('/');
        if p.is_empty() { "ecto_ledger" } else { p }.to_string()
    };
...
fn app_data_dir() -> PathBuf {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        PathBuf::from(appdata).join("ecto-ledger")
    }
