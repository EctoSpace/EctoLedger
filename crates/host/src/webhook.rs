...
fn format_cef(event: &EgressEvent) -> String {
    ...
    format!(
        "CEF:0|Ecto Ledger|1.0|observation_policy|{rule}|{sev}|session={sid} label={label} preview={prev}",
...
fn format_leef(event: &EgressEvent) -> String {
    format!(
        "LEEF:2.0|Ecto Ledger|1.0|observation_policy|\tsrc=ecto-ledger\tsessionId={sid}\tseverity={sev}\truleLabel={label}\tobservation={prev}",
        sid = event.session_id,
        sev = event.severity,
        label = event.rule_label,
        prev = event.observation_preview.replace('\t', " "),
