...
//   - See docs/sandbox-parity.md and docs/sandbox-verification-report.md for details.
...
pub enum SandboxError {
    Io(std::io::Error),
    /// The sandbox is not available on this platform (advisory-only).
    /// Callers that tolerate degraded security may log and continue;
    /// production systems should hard-fail.
    Unsupported(String),
    #[cfg(all(target_os = "linux", feature = "sandbox"))]
...
// Placeholder for the rest of sandbox.rs to be preserved unchanged
